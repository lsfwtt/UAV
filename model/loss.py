...
class SLSIoULoss(nn.Module):
    ...
    def forward(self, pred_log, target, warm_epoch, epoch, with_shape=True):
        ...
        if epoch>warm_epoch:       
            ...
            if with_shape:
                loss = (beta * (1 - siou_loss)).sum() + self.__LLoss(pred, target)
            else:
...
class MultiScaleSLSIoULoss(nn.Module):
    def __init__(self, base_loss, downsample):
        super().__init__()
        self.base_loss = base_loss
        self.downsample = downsample
    
    def forward(self, pred, masks, target, warm_epoch, epoch):
        total_loss = 0.0
        cur_target = target

        # 主输出loss
        total_loss += self.base_loss(pred, cur_target, warm_epoch, epoch)

        # 多尺度深监督
        for j, mask in enumerate(masks):
            if j > 0:
                cur_target = self.downsample(cur_target)
            total_loss += self.base_loss(mask, cur_target, warm_epoch, epoch)
        
        return total_loss / (len(masks) + 1)
